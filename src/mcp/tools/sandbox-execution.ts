...
export const sandboxExecutionTools: ToolModule[] = [
    ...
    definition: {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          maxOutputBytes: { type: "integer", minimum: 1000, maximum: 500000, description: "Overrides the profile's maxOutputBytes for this run. Also caps the total bytes of collectedArtifacts inlined when a cleanup policy removes the workspace." },
          collectArtifacts: {
            type: "array",
            maxItems: 100,
            items: { type: "string", minLength: 1, maxLength: 240 },
  ...
  },
  {
    definition: {
      ...
      inputSchema: { type: "object", properties: { sandboxId: sandboxIdProperty, outputPath: { type: "string", minLength: 1, maxLength: 240, description: "Relative path to write the Markdown report to, inside the sandbox — or inside a sibling <sandboxId>.reports/ directory once a cleanup policy has removed the workspace. Defaults to 'sandbox-report.md'." } }, required: ["sandboxId"], additionalProperties: false }
    },
    enabledByDefault: true,
