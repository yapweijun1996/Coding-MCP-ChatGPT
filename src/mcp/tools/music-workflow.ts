...
export const musicWorkflowTools: ToolModule[] = [
  ...
  {
    definition: { name: "apply_music_mix_master_chain", description: "Apply a deterministic preview mix/master chain to a WAV file with loudness normalization, limiter ceiling, stem notes, and an A/B mastering report.", inputSchema: { type: "object", properties: { projectId: { type: "string" }, audioPath: { type: "string" }, stemPaths: { type: "array", items: { type: "string" } }, chain: { type: "array", items: { type: "string", enum: ["room_ambience", "eq_cleanup", "gentle_compression", "limiter", "loudness_normalize"] } }, targetRms: { type: "number" }, truePeakCeiling: { type: "number" }, abLabel: { type: "string" }, outputAudioPath: { type: "string" }, outputReportPath: { type: "string" } }, required: ["projectId", "audioPath"], additionalProperties: false } },
    enabledByDefault: true,
