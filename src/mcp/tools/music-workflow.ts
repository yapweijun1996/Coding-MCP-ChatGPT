...
export const musicWorkflowTools: ToolModule[] = [
  ...
  },
  {
    ...
    handler: async (input, ctx) => {
      ...
      const manifestFile = await writeProjectFile(ctx.projectRoot, parsed.projectId, parsed.outputManifestPath, `${JSON.stringify(manifest, null, 2)}\n`);
      const errors = [...fileInspection.missingFiles.map((filePath) => `Missing export file: ${filePath}`), ...licenseWarnings.map((warning) => `License warning: ${warning}`), ...unsupportedFormats];
      return { ok: fileInspection.missingFiles.length === 0 && licenseWarnings.length === 0, summary: `Exported music package with ${fileInspection.exportedFiles.length} file(s), ${fileInspection.missingFiles.length} missing file(s), and ${licenseWarnings.length} license warning(s).`, jobId: parsed.projectId, previewUrl: published?.publishedUrl, shareUrl: published?.publishedUrl, artifacts: [htmlFile.path, manifestFile.path, readmeFile.path, playlistFile.path, packageReportFile.path], structuredContent: manifest, logs: [JSON.stringify(manifest, null, 2)], errors };
    }
  },
  {
