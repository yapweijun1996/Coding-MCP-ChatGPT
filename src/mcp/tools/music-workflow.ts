...
async function importMusicXmlScore(ctx: ToolContext, input: z.infer<typeof importMusicXmlScoreInputSchema>): Promise<MusicXmlImportResult> {
  ...
  for (const [partIndex, part] of parts.entries()) {
    ...
    for (const [measureIndex, measure] of measures.entries()) {
      ...
      // Notes are grouped by <voice> so each voice gets its own beat cursor starting at
      // measureStartBeat. MusicXML uses <backup>/<forward> to move the cursor between voices
      // within a measure; we don't parse those elements directly (fast-xml-parser doesn't
      // preserve cross-tag document order), but grouping by <voice> reproduces the same result
      // for the common case: voice 2's notes align with voice 1 instead of trailing after it.
      const notesByVoice = new Map<string, Record<string, unknown>[]>();
      for (const note of asArray(measure.note as Record<string, unknown> | Record<string, unknown>[] | undefined)) {
        const voice = textValue(note.voice) ?? "1";
        if (!notesByVoice.has(voice)) notesByVoice.set(voice, []);
        notesByVoice.get(voice)!.push(note);
      }

      let measureEndBeat = measureStartBeat;
      for (const voiceNotes of notesByVoice.values()) {
        let voiceBeat = measureStartBeat;
        let voiceLastNoteStartBeat = measureStartBeat;
        for (const note of voiceNotes) {
          ...
          const startBeat = isChord ? voiceLastNoteStartBeat : voiceBeat;
          if (!isChord) voiceLastNoteStartBeat = startBeat;
          if (!isRest) {
          ...
          if (!isChord) voiceBeat += durationBeats;
        }
        measureEndBeat = Math.max(measureEndBeat, voiceBeat);
      }
      beat = measureEndBeat;
